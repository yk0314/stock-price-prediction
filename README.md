...
- **市場相対強度**（`src/market.js`）: TOPIX専用エンドポイント(`/v2/indices/bars/daily/topix`)をfrom/to範囲指定で取得し、個別銘柄の20営業日騰落率との差分（相対強度）を計算します。無料枠への影響は軽微です。
...
**財務データのフィールド名**: `/v2/fins/summary`はEDINET XBRLの「冗長ラベル」がキーになっており、正式なキー名を公式ドキュメントで完全確認できていません。`src/financials.js`のCANDIDATE_KEYSは推測を含むため、実行後に`data/financials.json`を確認し、必要に応じて調整してください。
